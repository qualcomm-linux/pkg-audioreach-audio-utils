...
const struct StringToEnum halModFiles[] = {
    ...
    {"agm.c", AGM_MOD_FILE_AGM_SRC},
    {"device.c", AGM_MOD_FILE_DEVICE},
    {"device_hw_ep.c", AGM_MOD_FILE_DEVICE_HW_EP},
    {"graph.c", AGM_MOD_FILE_GRAPH},
    {"graph_module.c", AGM_MOD_FILE_GRAPH_MODULE},
    {"metadata.c", AGM_MOD_FILE_METADATA},
    {"session_obj.c", AGM_MOD_FILE_SESSION_OBJ},
    {"agm_callback.c", AGM_MOD_FILE_AGM_CALLBACK},
    {"agm_death_notifier.c", AGM_MOD_FILE_AGM_DEATH_NOTIFIER},
    {"agm_server_daemon.c", AGM_MOD_FILE_AGM_SERVER_DAEMON},
    {"agm_server_wrapper.c", AGM_MOD_FILE_AGM_SERVER_WRAPPER},
    {"ipc_proxy_server.c", AGM_MOD_FILE_IPC_PROXY_SERVER},
    {"agm_client_wrapper.c", AGM_MOD_FILE_AGM_CLIENT_WRAPPER},
    {"agm_compress_plugin.c", AGM_MOD_FILE_AGM_COMPRESS_PLUGIN},
    {"agm_dummy_impl.c", AGM_MOD_FILE_AGM_DUMMY_IMPL},
    {"agm_mixer_plugin.c", AGM_MOD_FILE_AGM_MIXER_PLUGIN},
    {"agm_pcm_plugin.c", AGM_MOD_FILE_AGM_PCM_PLUGIN},
};
                          ...
                          const XML_Char **attr)
{
    ...
    if (new_module) {
        ...
        if (strncmp(attr[1], data->mod_name, MAX_MODULE_NAME_LEN)) {
    ...
    if (attr[0] && !strncmp(attr[0], "name", sizeof("name"))) {
        ...
        else if (strncmp(data->mod_name, "agm", strlen("agm")) == 0) {
            index = stringToValue(halModFiles, ARR_SIZE(halModFiles), attr[1]);
            if (index < 0 || index >= HAL_MOD_FILE_MAX) {
                ALOGE("error in value of file returned");
                goto done;
