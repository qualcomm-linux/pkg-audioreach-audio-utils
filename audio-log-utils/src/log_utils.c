...
static int read_module(void) {
    ...
    while (j < count) {
        ...
        else if (!strncmp(buf[j], module_name[3], strlen(buf[j]))) {
            if (parse_log_xml(module_name[3], hal_log_mask, sizeof(hal_log_mask)/(sizeof(int))) != 0) {
                ALOGE("%s: could not parse %s logmask", __func__, module_name[3]);
            }
            j++;
        }
    }
...
static void *monitor_mod_file_change(void *context __UNUSED)
{
    ...
    ALOGD("%s", __func__);
    /* Initializing array elements with '0' */
    memset(wake_fds, 0, sizeof(wake_fds));
    ret = pipe(wake_fds);
    if (ret) {
